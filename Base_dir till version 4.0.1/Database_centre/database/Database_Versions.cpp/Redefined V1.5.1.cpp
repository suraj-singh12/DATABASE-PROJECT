...
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  ...
  * ::: Advanced Features :::
  ...
  NOTE: FUNCTION RETURNING 0 AS SUCCESS AND 1 AS NOT-SUCCESS
*/
#include<iostream>
#include<cstring>
#include<fstream>
...
};          //learn the code but don't change the order, since doing so will dismatch all the color orders & this will work in incorrect way
...
/********************************************************************************/
    ...
    void about();                                   //NEW
       inline void loop(char *temp_str)             //NEW
        {
            for(int temp_i=0;temp_str[temp_i]!='\0';++temp_i)
    ...
    void data_edit_start();
        void edit_data();      //arguments needed
...
/*************************************************************************************/
    ...
    //to change window size                         //NOT USED YET
    void window(int,int);

    //changes the buffer screen size                //NOT USED YET
    void Buffer();
...
/*******************************************************************************************************************************************************/
int main()      //HAHA this is main() lol! Have a look
{
    cout<<WHITE;
    data.call_main();
    return 0;
...
void student::about()
{
    ...
    char abt_str3[400]="It Has Ability To Discard Wrong Inputs\nFully Secured\nIt Gives You Independent Access To Your Database\nThe Students Can Only Add And Access Their Own Database Record i.e. They Need Master Password To Change Their Database!\nThe Master Password Cannot Be Changed By User Only The Developer Can Change It\n";
    for(int i=0;i<120;++i)
    {
        cout<<":";
    }
    cout<<"\n\t\t\t\t\t\tProgram Version: 1.5.1-R";
    cout<<CYAN<<"\n\nDescription \n\n";
    Sleep(100);
...
void student::call_main()
{
    ...
    cout<<setw(20)<<""<<"3.DELETE STUDENT DATA"<<endl;
    cout<<setw(20)<<""<<"4.EDIT STUDENT DETAILS"<<endl;
    cout<<setw(20)<<""<<"5.FORGOTTEN PASSWORD?"<<endl;
    cout<<setw(20)<<""<<"6.ABOUT"<<endl;
    cout<<setw(20)<<""<<"7.EXIT()"<<endl<<endl;
    ...
    if(choice==1)
        {
            cout<<"\n\t::::: Loading!:::::\n";
            MakeDocument();
            system("timeout>nul  /nobreak");   // It Stops the Program Execution For A While Without Displaying the Timer
        }
        call_main();
}
/*********************************************************************************************************************************************************/
void student::get_data()
{
    ...
    while(admission_date[0]=='\0'||check_date(admission_date)!=0||check_avail(admission_date)!=0)
    ...
    while(birth_date[0]=='\0'||check_date(birth_date)!=0||birth_date[9]==admission_date[9]||check_equality_dates(birth_date,admission_date)==0)
    ...
    while(char_height[0]=='\0'||check_int(char_height)!=0||int_len(char_height)>3||int_len(char_height)<=2)
...
void student::get_details()
{
    ...
    cout<<setw(7)<<"|>"<<"Date of Birth(DD/MM/YYYY)"<<setw(6)<<""<<":>"; gets(birth_date);
    while(birth_date[0]=='\0'||check_date(birth_date)!=0)
    { cout<<"\a"; gotoXY(x_cord,y_cord); cout<<setw(100)<<""; gotoXY(x_cord,y_cord); gets(birth_date);}
    ++y_cord;
...
void student::data_edit_start()
{
    ...
    cout<<setw(10)<<"|> "<<"Password (OF Length 8)"<<setw(9)<<":> ";
    strcpy(password,get_pass(password,8,x_cord,y_cord));
    ++y_cord;
...
void student::edit_data()
{
    ...
    switch(Choice)
    {
        case 1: cout<<setw(7)<<""<<"\nPlease Fill Up The Following Details Carefully\n\tENTER THE NEW DETAILS"<<endl;
                ...
                while(name[0]=='\0'||value_of(name)!=0)
                { gotoXY(x_cord,y_cord); cout<<setw(80)<<"";  gotoXY(x_cord,y_cord); gets(name); }

                strcpy(temp_name,"name");
                strcpy(name,all_upper(name));
                strcpy(name,to_underscore(name));
        ...
        case 2: cout<<setw(7)<<""<<"\nPlease Fill Up The Following Details Carefully\n\tENTER THE NEW DETAILS"<<endl;
                ...
                if(value_of(clas)>10)
                {
                    ...
                    while(char_result_10[0]=='\0'||(value_of(char_result_10)<33)||(value_of(char_result_10)>100))
                    { cout<<'\a'; gotoXY(x_cord,y_cord); cout<<setw(80)<<""; gotoXY(x_cord,y_cord); gets(char_result_10); }
...
/*---------------____________________________________________________________________------------------------------*/
                if(value_of(clas)<10)
                {
                    ...
                    //lets add the location of desired file
                    strcat(del,"database\\");
                ...
                //path of the file with name ready, lets execute the command & delete the class_10 result file.
                    system(del);        //executing the command
                }
/*---------------____________________________________________________________________------------------------------*/
                ...
                if(value_of(clas)<12)
                {
                    ...
                    strcpy(del,"ERASE /F /Q ");     //is a command to delete the file(even read -only also) using command prompt by forcefully in quite mode
                    //lets add the location of desired file
                    strcat(del,"database\\");
                    strcat(del,unique_id);
                    strcat(del,"_");
                    ...
                    strcat(del,temp);
                    strcat(del,".dbm");
                    //path of the file with name ready, lets execute the command & delete the class_12 result file.
                    system(del);        //executing the command
                }
/*---------------____________________________________________________________________------------------------------*/
                /*******************************
        ...
        case 4: cout<<setw(7)<<""<<"\nPlease Fill Up The Following Details Carefully\n\tENTER THE NEW DETAILS"<<endl;
                ...
                strcpy(temp_fathers_name,"fathers_name");
                strcpy(fathers_name,all_upper(fathers_name));
                strcpy(fathers_name,to_underscore(fathers_name));
        ...
        case 5: cout<<setw(7)<<""<<"\nPlease Fill Up The Following Details Carefully\n\tENTER THE NEW DETAILS"<<endl;
                ...
                while(mothers_name[0]=='\0'||value_of(mothers_name)!=0)
                { cout<<'\a'; gotoXY(x_cord,y_cord); cout<<setw(80)<<""; gotoXY(x_cord,y_cord); gets(mothers_name); }

                strcpy(temp_mothers_name,"mothers_name");
                strcpy(mothers_name,all_upper(mothers_name));
                strcpy(mothers_name,to_underscore(mothers_name));
                make_file(temp_mothers_name,mothers_name);
                edit_data();
                break;
        ...
        case 10:{
                    char temp[20],spell[64],ad_date[20];    int i=0;
                    strcpy(spell,"database\\");
                    strcat(spell,unique_id);
                    strcat(spell,"_");
                    ...
                    cout<<setw(9)<<"|> "<<"Date of Birth(DD/MM/YYYY)"<<setw(6)<<":> "; gets(birth_date);
                    while(birth_date[0]=='\0'||check_date(birth_date)!=0||check_equality_dates(birth_date,ad_date)==0)
                    { cout<<'\a'; gotoXY(x_cord,y_cord); cout<<setw(80)<<""; gotoXY(x_cord,y_cord); gets(birth_date); }
        ...
        case 11:
                ...
                strcpy(temp_address,"address");
                strcpy(address,all_upper(address));
                strcpy(address,to_underscore(address));
        ...
        case 12:
                cout<<setw(7)<<""<<"\nPlease Fill Up The Following Details Carefully\n\tENTER THE NEW DETAILS"<<endl;
                cout<<setw(9)<<"|> "<<"Category(SC/ST/OBC/GEN)"<<setw(4)<<":> "; gets(cast);
                while(cast[0]=='\0'||check_cast(cast)!=0)
        ...
        case 16:
                ...
                cout<<setw(9)<<"|> "<<"Student's Unique ID"<<setw(12)<<":> "; gets(unique_id);
                while(unique_id[0]=='\0'||check_int(unique_id)!=0||int_len(unique_id)!=15)
                { cout<<'\a'; gotoXY(x_cord,y_cord); cout<<setw(80)<<""; gotoXY(x_cord,y_cord); gets(unique_id); }
...
/*______________________________________________________________----------------------------------------____________________________________*/
        case 18:
                ...
                prev_loc:
                cout<<setw(7)<<""<<"\nPlease Fill Up The Following Details Carefully\n\tENTER THE NEW DETAILS"<<endl;
                cout<<setw(9)<<"|> "<<"First Provide With Class"<<setw(14)<<":> ";gets(clas);
                while(clas[0]=='\0'||clas[2]!='\0'||is_digit_alpha(clas)!=0)
                { cout<<'\a'; gotoXY(x_cord,y_cord); cout<<setw(80)<<""; gotoXY(x_cord,y_cord); gets(clas); }

                if(strcmp(temp_clas,clas)!=0)
                {
                    cout<<setw(7)<<""<<"\n\n\tPLEASE ENTER THE CORRECT CLASS-INFORMATION IN ORDER TO CONTINUE......\n";
                    gotoXY(x_cord,y_cord); cout<<setw(400)<<"";
                    goto prev_loc;
                }
                ...
                else { if(value_of(clas)<13){ strcat(clas,"th"); } }
